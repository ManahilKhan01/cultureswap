...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (path === "create-meeting") {
      ...
      if (!start_time) {
        return new Response(
          JSON.stringify({ error: "Missing required field: start_time" }),
          {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          },
      ...
      const endTime = new Date(
        new Date(start_time).getTime() + (duration_minutes || 60) * 60000,
      ).toISOString();
      ...
      if (!eventData.hangoutLink) {
        ...
        return new Response(
          JSON.stringify({
            error:
              "Failed to generate Google Meet link. Please check your Google Calendar settings.",
            details: eventData,
          }),
          {
            status: 500,
