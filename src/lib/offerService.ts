...
export const offerService = {
    ...
    async createOffer(offerData: OfferData): Promise<Offer | null> {
        try {
            ...
            await supabase.from('notifications').insert([{
                ...
                data: { conversation_id: offerData.conversation_id, offer_id: data.id },
                read: false
            }]);
     ...
     */
    async acceptOffer(offerId: string): Promise<{ offer: Offer; swap: any } | null> {
        try {
            const { data: { user } } = await supabase.auth.getUser();
            if (!user) throw new Error('Not authenticated');
            ...
            await supabase.from('notifications').insert([{
                ...
                data: { swap_id: updatedSwap.id, offer_id: offerId, conversation_id: offer.conversation_id },
                read: false
            }]);
    ...
    /**
     * Subscribe to offers received by the current user
     */
