...
export const ChatMessage = ({
  ...
  return (
    <div className="flex mb-5 group animate-in fade-in slide-in-from-bottom-2 duration-300 w-full items-start gap-4">
      {/* Avatar */}
      <div className="flex-shrink-0 mt-1">
        <div className="relative">
          <img
            ...
            className={`h-9 w-9 rounded-full object-cover shadow-sm ring-1 ${isAssistant ? "ring-blue-400 bg-blue-100" : "ring-border/30"}`}
          />
          {isAssistant && (
            <div className="absolute -bottom-0.5 -right-0.5 h-3.5 w-3.5 rounded-full bg-blue-500 border border-white flex items-center justify-center">
      ...
      <div className="max-w-[85%] md:max-w-[70%] flex flex-col items-start gap-1">
        {/* Name and Time (Optional, but adds to premium feel) */}
        {!isAssistant && (
        ...
        {message.content && message.content.trim() && (
          <div className="relative rounded-2xl rounded-tl-none px-4 py-3 bg-white border border-border/10 shadow-sm hover:shadow-md transition-shadow duration-300">
            <p className="text-[14.5px] leading-relaxed whitespace-pre-wrap font-medium text-foreground">
              {message.content}
            </p>
          </div>
        )}
        ...
        {attachments.length > 0 && (
          <div className="mt-2 space-y-3 flex flex-col items-start w-full">
            {attachments.length > 1 && onDownloadAll && (
              <Button
                variant="outline"
                size="sm"
                onClick={() => onDownloadAll(attachments)}
                className="h-8 text-[11px] font-bold tracking-tight rounded-lg bg-white/80 border-border/50 hover:bg-terracotta/5 hover:text-terracotta transition-all"
              >
            ...
            <div
              className={`grid gap-3 w-full ${attachments.length > 1 ? "grid-cols-1 sm:grid-cols-2" : "grid-cols-1"}`}
            >
