...
export const useUnreadMessages = (userId: string | null) => {
  ...
  const refreshUnreadCounts = useCallback(async () => {
    ...
    try {
      ...
      for (const msg of unreadMessages || []) {
        ...
        if (convId) {
          counts.byConversation[convId] =
            (counts.byConversation[convId] || 0) + 1;
        }
      }
  ...
  const markConversationAsRead = useCallback(
    async (conversationId: string) => {
      ...
      // Skip for assistant conversations (virtual IDs)
  ...
  useEffect(() => {
    ...
    const readStatusChannel = supabase
      ...
      .on(
        "postgres_changes",
        {
          event: "UPDATE", // Listen for updates (read status change)
          schema: "public",
          table: "messages",
        },
        (payload) => {
    ...
    // Polling backup (every 5 seconds) to ensure count is accurate
